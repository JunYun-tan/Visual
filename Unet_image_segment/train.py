...
from image_enhance import Transform_Compose, Train_Transform, Totensor, Test_Transform
...
# --------------------------------
# 指向数据集储存位置
parser.add_argument('--root', default='C:/Users/86157/Desktop/weizmann_horse_db', help='folder to Dataset')
...
parser.add_argument('--cut', default=True)
# 学习率
parser.add_argument('--lr', type=float, default=1e-3, help='the learning rate')
# 最小学习率
parser.add_argument('--min_lr', type=float, default=1e-6, help='minimum learning rate')
# 保存模型条件    达到此要求会保存一个模型参数 best_biou.pth
parser.add_argument('--early_stop_b_iou', type=float, default=0.5, help='the minimal boundary iou')
# 保存模型条件    达到此要求会保存一个模型参数 best_iou.pth
parser.add_argument('--early_stop_iou', type=float, default=0.70, help='the minimal iou')
# 退出训练条件之一 达到此要求代表有机会可以停止训练
parser.add_argument('--signal_iou', type=float, default=0.85, help='the signal iou')
...
# 设定随机种子
parser.add_argument('--manualSeed', type=int, default=9, help='manual seed')
...
def save_fig(TRAIN_IOU, TRAIN_LOSS, TEST_IOU, TEST_LOSS, LR, EPOCH, TRAIN_B_IOU, TEST_B_IOU):
    ...
    plt.xlabel('epoch')
    plt.ylabel('MIoU')
    plt.plot(EPOCH, TRAIN_IOU, 'b', label='Train')
    plt.plot(EPOCH, TEST_IOU, 'r', label='Test')
    plt.legend(loc='upper left')
    if plot_all == 1:
        plt.show()
    else:
        plt.savefig(r"./%s/mIoU.png" % (opt.outf))
    plt.clf()

    plt.figure(figsize=(8, 8))
    plt.title('Mean Boundary_Iou')
    plt.xlabel('epoch')
    plt.ylabel('Mean Boundary Iou')
    plt.plot(EPOCH, TRAIN_B_IOU, 'b', label='Train')
    plt.plot(EPOCH, TEST_B_IOU, 'r', label='Test')
    plt.legend(loc='upper left')
    if plot_all == 1:
        plt.show()
    else:
...
def show_predict(test_data_loader, sign):
    ...
    best_model = Unet_block(deep_supervision=deep_supervision, cut=cut)  # 默认在CPU上
    ...
    with torch.no_grad():
        for test_data, test_mask in test_data_loader:
            ...
            for k in range(B):
                ...
                pic1.save(r"./%s/%d_%d_origin.png" % (opt.predict, cal, k))
...
def main():
    ...
    if new_model == 1:
        print("新建模型")
        model = Unet_block(input_channel=3, num_classes=1, deep_supervision=deep_supervision, cut=False)  # 默认在cpu上

    elif new_model == 0:
        print("载入历史最佳模型")
        model = Unet_block(input_channel=3, num_classes=1, deep_supervision=deep_supervision, cut=False)
    ...
    for epoch in range(epoch_num):
        ...
        if test_mean_iou > best_iou:
            ...
            torch.save(model, 'Unet_block.pth')
            break_sign = 0
        ...
        # 停止训练条件
        if 0 < early_stopping < break_sign and best_iou > signal_iou:
            print("达到要求，停止训练")
            break
